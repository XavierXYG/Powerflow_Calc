...
def Interface(input):  # 输入为三个表达式向量
    ...
    if ~np.any(result_PV):
        result = np.r_[result_PQ[0]]
    elif ~np.any(result_PQ):
        result = np.r_[result_PV[0]]
    else:
        result = np.r_[result_PQ[0], result_PV[0]]
    return result
